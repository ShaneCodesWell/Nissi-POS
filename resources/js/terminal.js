document.addEventListener("DOMContentLoaded", async () => {
    ...
    function renderCategories() {
        ...
        categories.forEach((cat) => {
            ...
            btn.className =
                "category-btn px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-300 text-sm font-medium whitespace-nowrap hover:bg-slate-700/50 transition-all";
    ...
    async function updateQuantity(saleItemId, quantity) {
        ...
        try {
            const data = await api.patch(
                `/sales/${currentSale.id}/items/${saleItemId}`,
                { quantity },
            );
            currentSale = data.sale;
            renderCart();
    ...
    async function searchCustomers(term) {
        ...
        try {
            const data = await api.get(
                `/customers/search?q=${encodeURIComponent(term)}`,
            );
            const customers = data.customers || [];
            renderCustomerResults(customers, term);
        } catch (err) {
            console.warn("Customer search failed:", err.message);
        }
    }

    function renderCustomerResults(customers, searchTerm = "") {
        ...
        if (customers.length === 0) {
            container.innerHTML = `
                <div class="text-center py-4">
                    <p class="text-slate-500 text-sm mb-3">No customer found for "${searchTerm}"</p>
                    <button
                        onclick="window.__createCustomer('${searchTerm.replace(/'/g, "\\'")}')"
                        class="px-4 py-2 bg-emerald-500/20 border border-emerald-500/50 rounded-lg
                               text-emerald-400 text-sm hover:bg-emerald-500/30 transition-all">
        ...
        container.innerHTML = customers
            .map(
                ...
                <div class="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl
                            hover:bg-slate-700/50 cursor-pointer transition-all"
                     onclick="window.__attachCustomerFromSearch(${c.id})">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full bg-emerald-500/20 flex items-center
                                    justify-center text-emerald-400 text-sm font-semibold">
                            ${c.initials}
                        </div>
                        <div>
                            <p class="text-white text-sm font-medium">${c.name}</p>
                            <p class="text-slate-400 text-xs">${c.phone || ""}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        ${c.loyalty_tier ? `<span class="text-xs px-2 py-0.5 rounded-full bg-slate-600/50 text-slate-300">${c.loyalty_tier}</span>` : ""}
                        <p class="text-emerald-400 text-xs mt-1">${c.points_balance} pts</p>
                    </div>
                </div>`,
            )
            .join("");
    ...
    async function createCustomer(prefillName = "") {
        const { value: formValues } = await Swal.fire({
            title: "New Customer",
            html: `
                <div class="text-left space-y-4">
                    <div>
                        <label class="block text-gray-600 text-sm mb-1">First Name <span class="text-red-500">*</span></label>
                        <input id="swal-first-name" type="text" value="${prefillName}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400"
                            placeholder="e.g. Kwame" />
                    </div>
                    <div>
                        <label class="block text-gray-600 text-sm mb-1">Last Name</label>
                        <input id="swal-last-name" type="text"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400"
                            placeholder="e.g. Mensah" />
                    </div>
                    <div>
                        <label class="block text-gray-600 text-sm mb-1">Phone <span class="text-red-500">*</span></label>
                        <input id="swal-phone" type="tel"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400"
                            placeholder="+233 24 000 0000" />
                    </div>
                    <div>
                        <label class="block text-gray-600 text-sm mb-1">Email <span class="text-gray-400 text-xs">(optional)</span></label>
            ...
            showCancelButton: true,
            confirmButtonText: "Create & Attach",
            confirmButtonColor: "#10b981",
            cancelButtonText: "Cancel",
            didOpen: () => {
                document.getElementById("swal-first-name").focus();
            },
            preConfirm: () => {
                const firstName = document
                    .getElementById("swal-first-name")
                    .value.trim();
                const lastName = document
                    .getElementById("swal-last-name")
                    .value.trim();
        ...
        try {
            ...
            // If a sale is open, attach the new customer immediately
            if (currentSale) {
                const saleData = await api.post(
                    `/sales/${currentSale.id}/customer`,
                    {
            ...
            // Show success alert first, then update the UI after dismissal
            // This prevents the customer panel flashing behind the SweetAlert
            await Swal.fire({
                icon: "success",
                title: "Customer Created!",
                html: `<strong>${customer.name}</strong> has been created and attached to this sale.<br>
                                     <span class="text-sm text-gray-500">They've been enrolled in the ${customer.loyalty_tier || "Bronze"} tier.</span>`,
                confirmButtonColor: "#10b981",
            });

            // UI updates after the cashier dismisses the alert
            renderCart();
            ...
            const message =
                err.errors?.phone?.[0] || err.errors?.email?.[0] || err.message;
            Swal.fire({
                icon: "error",
    ...
    function renderAttachedCustomer() {
        const panel = document.getElementById("attachedCustomer");
        const customer = currentSale?.customer;
        ...
        if (customer) {
            ...
            // Disable search while a customer is attached
            const searchInput = document.getElementById("customerSearch");
            if (searchInput) {
                searchInput.value = "";
                searchInput.placeholder = "Customer attached";
            ...
            if (searchInput) {
                ...
                searchInput.placeholder = "Search customer by name or phone...";
            }
        }
    }

    // Global handlers for inline onclick attributes
    window.__createCustomer = createCustomer;
    window.__updateQty = updateQuantity;
    window.__removeItem = removeItem;

    window.__attachCustomerFromSearch = async function (customerId) {
        if (!currentSale) {
            Swal.fire({
                icon: "info",
                title: "No active sale",
                text: "Add an item first, then attach a customer.",
            });
            return;
        }

        try {
            const data = await api.post(`/sales/${currentSale.id}/customer`, {
            ...
            showError("Could not attach customer", err.message);
        }
    };
    ...
    async function applyDiscount() {
        ...
        try {
            const preview = await api.post(
                `/sales/${currentSale.id}/discount/preview`,
                {
                    code: result.value,
                },
            );
    ...
    function openCashModal() {
        const total = parseFloat(currentSale.total_amount);
        document.getElementById("cashAmountDue").textContent =
            `GHS ${total.toFixed(2)}`;
    ...
    async function processCashPayment() {
        ...
        try {
            // Step 1 — record cash payment
            const paymentData = await api.post(
                `/sales/${currentSale.id}/payments/cash`,
                {
                    amount: tendered,
                },
            );
    ...
    function setupEventListeners() {
        ...
        document
            ...
            .addEventListener("click", removeDiscount);
        document
        ...
        document
            .getElementById("completeCashPayment")
            .addEventListener("click", processCashPayment);
        ...
        document
            ...
            .addEventListener("input", function () {
                ...
                const total = currentSale
                    ? parseFloat(currentSale.total_amount)
                    : 0;
        ...
        // Detach customer
        document
            .getElementById("detachCustomerBtn")
            ?.addEventListener("click", async () => {
                ...
                try {
                    ...
                    showError("Could not detach customer", err.message);
                }
            });
        ...
        if (customerSearchInput) {
            ...
            customerSearchInput.addEventListener("input", (e) => {
                ...
                searchTimeout = setTimeout(() => searchCustomers(term), 400);
            });
        }
